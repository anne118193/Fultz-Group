...
def cmd_run(T, V, cut, side, RunSteps, traj, n, seed=-1):
    ...
    inp_line='{} -v SEED {} -v T {} -v c {} -v SIDE {} -v TRAJ {}'.format(system_line, seed, T, cut, side, traj)   
    print(path1)
    ...
    if path.exists(path1) and len(max_existed_step) >= 1:
        ...
        os.mkdir(f"{out_path}")
        os.mkdir(f"{path1}")
        shutil.copy2("supercell_lammps", f"{out_path}/supercell_lammps")
        f = open(f"{out_path}/seed.txt", 'a')
        f.write(f"{T} {V} {cut} {side} {seed}\n")
        f.close()

        a = V**(1/3.0)
        sp.run(f"sed -i 's|lat_param|{a}|g' {out_path}/supercell_lammps", shell=True)
        sp.run(f"sed -i 's|half|{a/2}|g' {out_path}/supercell_lammps", shell=True)

        inp = inp_line + ' -v RUNSTEPS {} -in SOL.in'.format(RunSteps) 
        inp_E0 = f'{system_line} -v SIDE {side} -v SEED {seed} -in E0.in'
        return [inp, inp_E0]
